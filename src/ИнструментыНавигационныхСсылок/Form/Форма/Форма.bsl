...
Процедура ЗаполнитьКодыПолученияСсылок()
	...
	ФункцияАналитикиСсылка	 = ТекстФункцииСсылка(СсылкаНаОбъект);
...
&НаКлиенте
Процедура СкопироватьФормулуСсылки(Команда)
	ТекстВБуферОбмена(ФункцияАналитикиСсылка);
КонецПроцедуры
...
//
&НаСервереБезКонтекста
Функция ТекстФункцииСсылка(СсылкаНаОбъект)

	ВозвращаемоеЗначениеПоУмолчанию = "";
	
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект)
		Или Не ЭтоСсылка(СсылкаНаОбъект) Тогда
		Возврат ВозвращаемоеЗначениеПоУмолчанию;
	КонецЕсли; 
	...
	ШаблонФункции = "ССЫЛКА(""%1"", ""%2"", ""%3"")";
	ТекстФункции = СтрШаблон(ШаблонФункции,
...
КонецФункции // ТекстФункцииСсылка()
